...
# from dianping.items import reviewItem
...
class DianpingSpider(CrawlSpider):
    ...
    # start_urls=[
    #     'http://www.dianping.com/memberlist/1/0'
    ...
    def start_requests(self):
        # urls = ['http://www.dianping.com/memberlist/{}'.format(str(i)) for i in range(1, 10)]
        for i in range(1,50):
            url = 'http://www.dianping.com/memberlist/%d' %i
            yield Request(url,headers= self.headers,dont_filter=False)
    ...
    def parse(self, response):
        ...
        for info in infos:
            ...
            # 点评数
            # commentNum = info.xpath('td[2]/text()').extract()
            #详情页url
            ...
            # 鲜花数
            flower = info.xpath('td[5]/text()').extract()
            if len(nickname)>0 :
                name =  nickname[0]
                detailurl =  'http://www.dianping.com'+url[0]
                reviewsUrl = detailurl + '/reviews'
                wishlistUrl = detailurl + '/wishlists'
                chekinUrl = detailurl + '/checkin'
                ...
                # yield Request(reviewsUrl,callback=self.review_details,headers= headers )
                # yield Request(wishlistUrl,callback=self.wishlist_details,headers = headers)
        ...
        next_page = selector.xpath('//div[@class="Pages"]/a/@href')
        if next_page:
            url = response.urljoin(next_page[4].extract())
            yield Request(url,callback=self.parse)
        # for p in range(2,7):
        #     next_url = str(url) +'?pg=%s' %p
        #
                ...
                if len(content) >0 :
                    ...
                    if item == '出生地：':
                       foodItem['birthPlace'] = content
                        ...
                        foodItem['movie'] = content
                    elif item == '音乐/歌曲/音乐人：':
                        foodItem['music'] = content
                    elif item == '书/作者：':
                        foodItem['book'] = content
                    elif item == '星座：':
    ...
    #
    #     selector = Selector(response)
    #     infos = selector.xpath('//div[@class="modebox p-term-list"]/ul')
    #
    #     typelist =''
    #     citylist =''
    #     for info in infos:
    #         type = info.xpath('li[1]/span/a/text()').extract()
    #         for typea in type:
    #             typelist +=typea + ';'
    #         item['type'] = typelist
    #         city = info.xpath('li[2]/span/a/text()').extract()
    #         for citya in city:
    #             citylist +=citya + ';'
    #         item['city'] = citylist
    #     yield item
